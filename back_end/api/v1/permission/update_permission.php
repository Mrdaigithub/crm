...
//判断要查询的权限组是否存在
if (!count($GLOBALS['sql']->query("SELECT id FROM admin_group WHERE id='$role_id'"))) error_handler(46008);
...
$permission_len = $GLOBALS['sql']->query("SELECT count(iid FROM pepermission;
$permission_len = $permission_len[0]["count('idid"];

//如果权限详情参数长度不够自动补全
if (strlen($new_permission) < $permission_len) $new_permission = str_pad($new_permission, $permission_len, '0');

$GLOBALS['sql']->exec("UPDATE admin_group SET pepermission$new_permission' WHERE idid$role_id'");
$permission_json = array();
for ($i = 0; $i < strlen($new_permission); $i++) {
    $role_permission;
    $p = $GLOBALS['sql']->query("SELECT ididname, des FROM pepermissionHERE ididi+1");
    $p_id = $p[0]['idid;
    $p_name = $p[0]['name'];
    $p_des = $p[0]['des'];
    $p_json = array(
